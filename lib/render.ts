...
function renderSection(
  ...
  sectionTable += "\r\n";
  sectionTable += `${headingPrefix} ${name}\r\n\n`; // section header
  if (description !== "") {
 ...
 */
export function insertReadmeTable(
  ...
  for (let i = 0; i < tree.length; i += 1) {
    ...
    if (i === tree.length - 1) {
      ...
      if (!ignoreDefault) {
        ...
        currentProperties[tree[i]].default = value.value;
      }
      if (value.nullable) {
        currentProperties[tree[i]].nullable = true;
      }
      if (value.type === "array") {
        // The last element of the tree is an array.
        // It is a plain or empty array since if not, the tree would have more elements
        if (value.value === null || (value.value as unknown[]).length === 0) {
          // When it is an empty array
          currentProperties[tree[i]].items = {}; // TODO: how do we know the type in empty arrays?
        } else {
          currentProperties[tree[i]].items = {
            type: typeof (value.value as unknown[])[0],
          };
        }
      }
    } else {
      ...
      if (isArray) {
        ...
        generateSchema(value, tree.slice(i + 1), currentProperties[key].items!.properties!, true);
      ...
      } else if (!Object.prototype.hasOwnProperty.call(currentProperties, key)) {
        currentProperties[key] = {
          type: "object",
          properties: {},
        };
      ...
      currentProperties = currentProperties[key].properties!;
    }
  }
}
...
function createValuesSchema(values: Parameter[]): {
  title: string;
  type: string;
  properties: SchemaProperties;
} {
  const schema = {
    title: "Chart Values",
    type: "object",
    properties: {} as SchemaProperties,
  };
 ...
 */
export function renderOpenAPISchema(
